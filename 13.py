...
def _cnn():
    ...
    train_data,validation_data,train_target,validation_target = train_test_split(train_data,train_target, test_size=0.2, random_state=42)# randomly split data into traingin and validation sets
    ...
    model.add(convolutional.Conv2D(64, (5,5), padding='same'))# 2nd convo layer
    model.add(Activation('relu'))# using relu for activation
    model.add(pooling.MaxPooling2D(pool_size=(2,2), padding='same'))# pooling

    model.add(convolutional.Conv2D(64, (5,5), padding='same'))# 3rd convo layer
    model.add(Activation('relu'))# using relu for activation
    model.add(pooling.MaxPooling2D(pool_size=(2,2), padding='same'))# pooling

    model.add(convolutional.Conv2D(64, (5,5), padding='same'))# 4th convo layer
    model.add(Activation('relu'))# using relu for activation
    model.add(pooling.MaxPooling2D(pool_size=(2,2), padding='same'))# pooling

    model.add(convolutional.Conv2D(64, (5,5), padding='same'))# 5th convo layer
    model.add(Activation('relu'))# using relu for activation
    model.add(pooling.MaxPooling2D(pool_size=(2,2), padding='same'))# pooling
    ...
    model.add(Activation('softmax'))# using softmax for activation
...
if __name__ == '__main__':
    ...
    with open('possible_submissions/thirteen.csv', 'wb') as f:
